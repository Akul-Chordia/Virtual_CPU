...
void two_complement(bool a[8], bool b[8], bool &negative){
    for (int i = 0; i<8; i++){
        b[i] = NOT(a[i]);
    }
...
void eight_bit_sub(bool a[8], bool b[8], bool cin, bool value[8], bool &negative){
    bool temp[8];
    two_complement(b, temp, negative);
    increment(temp, negative);
    eight_bit_adder(a, temp, cin, value, negative);
}
...
void eight_bit_ADD(bool a[8], bool b[8]){
    for (int i = 0; i<8; i++){
        a[i] = AND(a[i], b[i]);
    }
}
